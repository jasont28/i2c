...
static void i2c__kick_off_transfer(i2c_s *i2c, uint8_t slave_address, uint8_t starting_slave_memory_address,
  ...
  i2c->slave_register_address = starting_slave_memory_address;
...
static bool i2c__handle_state_machine(i2c_s *i2c) {
  ...
  case I2C__STATE_ARBRITRATION_LOST:
    ...
    //========================================================================
    //                      S L A V E  S T A T E S
    //========================================================================

    // List of executed state: 0x60, 0x80, 0xA0, 0xA8, 0xB0, 0xC0
  case 0x60:
  ...
  case 0x80:
    if (i2c->slave_address_received)
      ...
      I2C__DEBUG_PRINTF("  @0x80: RECEIVED SLAVE REGISTER ADDRESS: 0x%02X", i2c->slave_register_address);
    }
    //--(i2c->number_of_bytes_to_transfer);
    /* if (status) { // if it 1
       lpc_i2c->CONCLR = 0x0C; //It never goes here?????
       fprintf(stderr, "What about this?\n");
     } else {*/
    // fprintf(stderr, "Does it goes here?\n");
    i2c__set_ack_flag(lpc_i2c);
    i2c__clear_si_flag_for_hw_to_take_next_action(lpc_i2c);
    //}
    break;
